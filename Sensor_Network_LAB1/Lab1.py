...
def ble_scan(sock):
    ...
    for beacon in returnedList:
        ...
        if raw_uuid == "00000000000000000000000000811562":    # Change your uuid here
            print("--------")
            ...
            beacon.rssi = float(beacon.rssi)
            beacon.unknown = float(beacon.unknown)
            if (beacon.rssi/beacon.unknown) < 1:             # ratio < 1
                rssiDict = (beacon.rssi/beacon.unknown)**10
            else:            # ratio > 1
                rssiDict = coef1*(beacon.rssi/beacon.unknown)**coef2+coef3
